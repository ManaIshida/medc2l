...
from knowledge_ja import get_lexical_relations_from_preds
...
def make_axioms_from_preds(premise_preds, conclusion_pred, pred_args, model):
    axioms = set()
    linguistic_axioms = \
        get_lexical_relations_from_preds(
            premise_preds, conclusion_pred, pred_args, model)
    # ない
    print(linguistic_axioms)
    axioms.update(set(linguistic_axioms))
    return axioms


def try_abductions_dcs(theorem):
    ...
    while True:
        ...
        abduction_theorem = try_abduction(
            t_pos, previous_axioms=current_axioms, expected='yes', model='dcs')
        ...
        if theorem_master.result == 'unknown':
            abduction_theorem = try_abduction(
                t_neg, previous_axioms=current_axioms, expected='no', model='dcs')
...
def try_abductions_w2v(theorem):
    assert len(theorem.variations) == 2, 'Got %d != 2 variations' % len(theorem.variations)
    t_pos, t_neg = theorem.variations[:2] # theorem positive and negated.
    theorem_master = t_pos
    ...
    while True:
        # TODO: should be previous_axioms=axioms?
        abduction_theorem = try_abduction(
            t_pos, previous_axioms=current_axioms, expected='yes', model='w2v')
        # TODO: should we add this adbuction_theorem in theorem_master.variations?
        current_axioms = axioms.union(abduction_theorem.axioms)
        # TODO: should it be abduction_theorem.result == 'unknown'?
        if theorem_master.result == 'unknown':
            abduction_theorem = try_abduction(
                t_neg, previous_axioms=current_axioms, expected='no', model='w2v')
            theorem_master.variations.append(abduction_theorem)
            current_axioms.update(abduction_theorem.axioms)
        if len(axioms) == len(current_axioms) or theorem_master.result != 'unknown':
            break
...
def try_abductions_ginza(theorem):
    assert len(theorem.variations) == 2, 'Got %d != 2 variations' % len(theorem.variations)
    t_pos, t_neg = theorem.variations[:2] # theorem positive and negated.
    theorem_master = t_pos
    ...
    while True:
        # TODO: should be previous_axioms=axioms?
        abduction_theorem = try_abduction(
            t_pos, previous_axioms=current_axioms, expected='yes', model='ginza')
        # TODO: should we add this adbuction_theorem in theorem_master.variations?
        current_axioms = axioms.union(abduction_theorem.axioms)
        # TODO: should it be abduction_theorem.result == 'unknown'?
        if theorem_master.result == 'unknown':
            abduction_theorem = try_abduction(
                t_neg, previous_axioms=current_axioms, expected='no', model='ginza')
            theorem_master.variations.append(abduction_theorem)
            current_axioms.update(abduction_theorem.axioms)
        if len(axioms) == len(current_axioms) or theorem_master.result != 'unknown':
            break
...
def make_axioms_from_coq_analysis(failure_log, model):
    axioms = set()
    for subgoal in failure_log.get('other_sub-goals', []):
        ...
        premise_preds = failure_log.get('all_premises', [])
        pred_args = get_predicate_arguments(
            #subgoal.get('matching_raw_premises', []),
            failure_log.get('all_premises', []),
...
def try_abduction(theorem, previous_axioms=None, expected='yes', model='dcs'):
    ...
    axioms = make_axioms_from_coq_analysis(failure_log, model)
