...
def main(args = None):
    ...
    parser.add_argument("--abduction", nargs='?', type=str, default="no",
        choices=["no", "naive", "spsa", "dcs", "w2v", "ginza"],
    ...
    if ARGS.abduction == "spsa":
        ...
        ABDUCTION = AxiomsWordnet()
    elif ARGS.abduction == "dcs":
        from abduction_ja import AxiomsDCS
        ABDUCTION = AxiomsDCS()
    elif ARGS.abduction == "w2v":
        from abduction_ja import AxiomsW2V
        ABDUCTION = AxiomsW2V()
    elif ARGS.abduction == "ginza":
        from abduction_ja import AxiomsGINZA
        ABDUCTION = AxiomsGINZA()

    parser = etree.XMLParser(remove_blank_text=True)
    root = etree.parse(ARGS.sem, parser)
    #print(etree.tostring(root), file=sys.stdout)
    ...
    for doc, proof_node in zip(DOCS, proof_nodes):
        ...
        #print(etree.tostring(doc), file=sys.stdout)
